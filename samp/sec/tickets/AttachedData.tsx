...
type AttachedDataProps = ChildrenProps & {
...
const AttachedData = (props: AttachedDataProps) => {
  ...
  return (
    <>
      ...
      <div
        className={`absolute left-0 right-0 z-50 mx-auto flex w-[70%] flex-col rounded-t-base bg-primary-lighter p-4 transition-transform duration-300 ${isFileSelected ? 'bottom-10 h-[75px] -translate-y-10' : 'bottom-0 translate-y-0'} `}
      >
        <div className="relative flex w-full items-start justify-between">
          ...
          <div className={`tr-300 w-fit transform`}>
            {isUploadLoading ? (
              <CircularProgress className="mt-1" size={30} />
            ) : (
              <ProgressLabelBar
                className={`tr-300 ${ isUploadStarted ? 'visible block opacity-100' : 'invisible hidden opacity-0'}`}
        ...
        {previewUrl && (
          <>
            <div className="!absolute left-0 top-9 flex w-full items-center justify-end gap-x-1">
              {children}
            </div>
          </>
        )}
