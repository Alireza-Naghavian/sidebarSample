...
function VoiceManagmentProvider({ children }: ChildrenProps) {
  // states
  ...
  const checkMicPermission = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`خطا در بررسی مجوز میکروفون=> ${error}`);
      setIsMicPermGranted(false);
    }
  }, []);
  ...
  const cancelRecording = useCallback(() => {
    try {
      ...
      toast.error(`'Error canceling recording: => ${error}`);
    } finally {
      if (recordIntervalRef.current) {
        clearInterval(recordIntervalRef.current);
        recordIntervalRef.current = null;
...
export default VoiceManagmentProvider;
